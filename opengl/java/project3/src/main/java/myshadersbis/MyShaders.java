...
public class MyShaders extends GLCanvas implements GLEventListener {
    ...
    private Texture txt;
    private TextureData txtString;
    private TextureData txtData;
    private int txtIdString;
    private int txtId;
...
//            gl_.glGenerateMipmap(GL2.GL_TEXTURE_2D);
        ...
        String text_ = "TEXT";
        FontMetrics fm_ = getGraphics().getFontMetrics();
        int w_ = fm_.stringWidth(text_);
        int h_ = fm_.getHeight();
//        new Font("",Font.PLAIN,12).getF
        BufferedImage img_ = new BufferedImage(w_, h_, BufferedImage.TYPE_INT_ARGB);
        Graphics g_ = img_.createGraphics();
//        Color wh_ = Color.WHITE;
//        g_.setColor(new Color(wh_.getRed(),wh_.getGreen(), wh_.getBlue(), 0));
        g_.setColor(Color.WHITE);
        g_.fillRect(0, 0, w_, h_);
        g_.setColor(new Color(212, 175, 55));
        g_.drawString(text_, 0, h_ - 1);
//        transparentAllWhite(img_);
        try {
        	ByteArrayOutputStream baos_ = new ByteArrayOutputStream();
            ImageIO.write(img_, TextureIO.PNG, baos_);
            baos_.flush();
            ByteArrayInputStream bis_ = new ByteArrayInputStream(baos_.toByteArray());
            baos_.close();
            txtString = TextureIO.newTextureData(gl_.getGLProfile(), bis_, true, TextureIO.PNG);
            int[] textureId = new int[1];
            gl_.glGenTextures(1, textureId, 0);
            gl_.glBindTexture(GL2.GL_TEXTURE_2D, textureId[0]);
//            gl_.glTexImage2D(GL2.GL_TEXTURE_2D, 0, txtString.getInternalFormat(), txtString.getWidth(), txtString.getHeight(), 0, txtString.getPixelFormat(), GL2.GL_UNSIGNED_BYTE, txtString.getBuffer());
            gl_.glTexImage2D(GL2.GL_TEXTURE_2D, 0, GL2.GL_RGBA, txtString.getWidth(), txtString.getHeight(), 0, GL2.GL_RGBA, GL2.GL_UNSIGNED_BYTE, txtString.getBuffer());
            txtIdString = textureId[0];
            g_.dispose();
		} catch (Exception e_) {
			e_.printStackTrace();
		}
...
//        gl_.glFogf(GL2.GL_FOG_MODE, GL2.GL_EXP);
        ...
        _gl.glBlendFunc(GL2.GL_ONE_MINUS_DST_ALPHA,GL2.GL_DST_ALPHA);
        _gl.glBlendFunc(GL2.GL_SRC_ALPHA, GL2.GL_ONE_MINUS_SRC_ALPHA);
        _gl.glBindTexture(GL2.GL_TEXTURE_2D, txtIdString);
//        //Texture unit 0 is for base images.
//
        _gl.glVertexPointer(3, GL2.GL_FLOAT, 0, Buffers.newDirectFloatBuffer(quadVerticesBis));
...
//        _gl.glTexImage2D(GL2.GL_TEXTURE_2D, 0, txtString.getInternalFormat(), txtString.getWidth(), txtString.getHeight(), 0, txtString.getPixelFormat(), GL2.GL_UNSIGNED_BYTE, txtString.getBuffer());
        _gl.glTexImage2D(GL2.GL_TEXTURE_2D, 0, GL2.GL_RGBA, txtString.getWidth(), txtString.getHeight(), 0, GL2.GL_RGBA, GL2.GL_UNSIGNED_BYTE, txtString.getBuffer());
