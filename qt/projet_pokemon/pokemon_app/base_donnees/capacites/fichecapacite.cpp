...
FicheCapacite::FicheCapacite(const QStringList& _effets,Donnees* _d){
		...
		foreach(QString e,effets_){
			...
			if(e.contains("SI_CLIMAT")){
				...
				if(cles_.contains(infos_[0])){
					QStringList& liste_=immu_statut.valeur(infos_[0]);
				...
				QList<QString> cles_=immu_statut.cles();
				if(cles_.contains("")){
					QStringList& liste_=immu_statut.valeur("");
				...
				foreach(QString s,infos_){
					...
					foreach(QString c,cles_){
						QStringList& liste_=immu_statut.valeur(c);
						if(!liste_.contains(s)){
							liste_.append(s);
						}
					}
					foreach(QString c,_d->val_noms_climats()){
						QString cl_=c.split("\t")[_d->val_indice_francais()];
	...
	if(!effets_.isEmpty()){
		foreach(QString e,effets_){
			...
			if(cles_.contains(pair_)){
				QRegExp exp_taux_("[0-9]+(/[0-9]+)?");
				e.indexOf(exp_taux_);
				mult_puissance_type_si_climat.valeur(pair_)=Taux::parse_taux(exp_taux_.capturedTexts()[0]);
			}else{
				QRegExp exp_taux_("[0-9]+(/[0-9]+)?");
				e.indexOf(exp_taux_);
	...
	if(!effets_.isEmpty()){//TODO changer attribut
		foreach(QString e,effets_){
			...
			if(!cles_.contains(infos_[0])){
				...
				QList<QPair<QString,qint8> >& valeur_=mult_statistique_si_touche_categorie.valeur(infos_[0]);
				valeur_<<QPair<QString,qint8>("",0);
				valeur_.last().first=infos_[1];
				infos_[2].chop(1);
				valeur_.last().second=(qint8)infos_[2].toInt();
...
Taux FicheCapacite::taux_act(bool _neg)const{
	if(mult_variation_cran<Taux(0)){
		if(_neg){
			return mult_variation_cran.absolu();
		}
		return mult_variation_cran;
	}
	if(mult_variation_cran>Taux(0)){
...
QString FicheCapacite::description(int _langue,Donnees *_d)const{
	QString retour_;
	QString cle_;
	if(anti_climat){
		cle_="ANTI_CLIMAT";
	...
	if(!ign_capacite.isEmpty()){
		...
		foreach(QString c,ign_capacite){
			args_<<Utilitaire::traduire(_d->val_noms_capacites(),c,_langue);
		}
		args_.sort();
	...
	if(!ignore_attaque.isEmpty()){
		...
		foreach(QString c,ignore_attaque){
			args_<<Utilitaire::traduire(_d->val_noms_attaques(),c,_langue);
		}
		args_.sort();
		args_=QStringList(args_.join(","));
		retour_+=Utilitaire::formatter(_descriptions_capacites_.valeur(cle_).split("\t")[_langue],args_)+"\n";
	...
	if(!ignore_boost_statistique_adv.isEmpty()){
		...
		foreach(QString c,ignore_boost_statistique_adv){
			args_<<Utilitaire::traduire(_d->val_noms_statistiques(),c,_langue);
		}
		args_.sort();
		args_=QStringList(args_.join(","));
		retour_+=Utilitaire::formatter(_descriptions_capacites_.valeur(cle_).split("\t")[_langue],args_)+"\n";
	...
	if(!immu_attaque_type.isEmpty()){
		...
		foreach(QString c,immu_attaque_type){
			args_<<Utilitaire::traduire(_d->val_noms_types(),c,_langue);
		}
		args_.sort();
		args_=QStringList(args_.join(","));
		retour_+=Utilitaire::formatter(_descriptions_capacites_.valeur(cle_).split("\t")[_langue],args_)+"\n";
	...
	if(type_attaques!=""){
		cle_="TYPE_ATTAQUES";
		QStringList args_;
		args_<<Utilitaire::traduire(_d->val_noms_types(),type_attaques,_langue);
		retour_+=Utilitaire::formatter(_descriptions_capacites_.valeur(cle_).split("\t")[_langue],args_)+"\n";
	}
	...
	if(!cles_ch_.isEmpty()){
		...
		foreach(QString e,cles_ch_){
			QStringList args_;
			args_<<Utilitaire::traduire(_d->val_noms_climats(),e,_langue);
	...
	if(!cles_ch_.isEmpty()){
		...
		foreach(QString e,cles_ch_){
			QStringList args_;
			if(Utilitaire::traduisible(_d->val_noms_statuts(),e)){
				args_<<Utilitaire::traduire(_d->val_noms_statuts(),e,_langue);
			}else{
				args_<<Utilitaire::traduire(_d->val_noms_pseudo_statuts(),e,_langue);
			}
	...
	if(!cles_ch_.isEmpty()){
		...
		foreach(QString e,cles_ch_){
			QStringList args_;
			args_<<Utilitaire::traduire(_d->val_noms_attaques(),e,_langue);
		...
		foreach(QString e,cles_ch_){
			if(e==""){
				...
				foreach(QString v,immu_statut.valeur(e)){
					QStringList args_;
					if(Utilitaire::traduisible(_d->val_noms_statuts(),v)){
						args_<<Utilitaire::traduire(_d->val_noms_statuts(),v,_langue);
					}else{
						args_<<Utilitaire::traduire(_d->val_noms_pseudo_statuts(),v,_langue);
					}
					retour_+=Utilitaire::formatter(_descriptions_capacites_.valeur(cle_).split("\t")[_langue],args_)+"\n";
				}
			}else{
	...
	if(!anti_immu_attaque.isEmpty()){
		...
		foreach(TypeType e,anti_immu_attaque){
			...
			args_<<Utilitaire::traduire(_d->val_noms_types(),e.first,_langue);
	...
	if(!cles_ch_.isEmpty()){
		foreach(QString e,cles_ch_){
			...
			args_<<Utilitaire::traduire(_d->val_noms_climats(),e,_langue);
			args_<<Utilitaire::traduire(_d->val_noms_types(),changement_type_si_climat.valeur(e),_langue);
			retour_+=Utilitaire::formatter(_descriptions_capacites_.valeur(cle_).split("\t")[_langue],args_)+"\n";
		}
	}
	...
	if(!cles_ch_.isEmpty()){
		foreach(QString e,cles_ch_){
			...
			if(Utilitaire::traduisible(_d->val_noms_statuts(),statut_recu_)){
				args_<<Utilitaire::traduire(_d->val_noms_statuts(),statut_recu_,_langue);
			}else{
				args_<<Utilitaire::traduire(_d->val_noms_pseudo_statuts(),statut_recu_,_langue);
			}
			retour_+=Utilitaire::formatter(_descriptions_capacites_.valeur(cle_).split("\t")[_langue],args_)+"\n";
		}
	}
	...
	if(!cles_ch_.isEmpty()){
		...
		foreach(QString e,cles_ch_){
			foreach(ValPart v,mult_statistique_si_touche_categorie.valeur(e)){
				...
				args_<<Utilitaire::traduire(_d->val_noms_categories(),e,_langue);
				args_<<Utilitaire::traduire(_d->val_noms_statistiques(),v.first,_langue);
	...
	if(!cles_ch_.isEmpty()){
		foreach(QString e,cles_ch_){
			...
			args_<<Utilitaire::traduire(_d->val_noms_statistiques(),valeur_.first,_langue);
	...
	if(!cles_cpl_.isEmpty()){
		...
		foreach(CleChCh e,cles_cpl_){
			QStringList args_;
			args_<<Utilitaire::traduire(_d->val_noms_climats(),e.first,_langue);
			args_<<Utilitaire::traduire(_d->val_noms_types(),e.second,_langue);
	...
	if(!cles_cpl_.isEmpty()){
		...
		foreach(CleChCh e,cles_cpl_){
			...
			args_<<Utilitaire::traduire(_d->val_noms_categories(),e.second,_langue);
	...
	if(!cles_cpl_.isEmpty()){
		...
		foreach(CleChCh e,cles_cpl_){
			QStringList args_;
			args_<<Utilitaire::traduire(_d->val_noms_statistiques(),e.first,_langue);
			args_<<Utilitaire::traduire(_d->val_noms_climats(),e.second,_langue);
	...
	if(!cles_ch_.isEmpty()){
		foreach(QString e,cles_ch_){
			...
			args_<<Utilitaire::traduire(_d->val_noms_statistiques(),mult_statistique_si_statistique_adv.second.second,_langue);
			args_<<Utilitaire::traduire(_d->val_noms_statistiques(),mult_statistique_si_statistique_adv.second.first,_langue);
	...
	if(!mult_statistique_si_statut_cran.isEmpty()){
		...
		foreach(StatistiqueStatut e,mult_statistique_si_statut_cran){
			QStringList args_;
			if(Utilitaire::traduisible(_d->val_noms_statuts(),e.second.first)){
				args_<<Utilitaire::traduire(_d->val_noms_statuts(),e.second.first,_langue);
			}else{
				args_<<Utilitaire::traduire(_d->val_noms_pseudo_statuts(),e.second.first,_langue);
			}
			args_<<Utilitaire::traduire(_d->val_noms_statistiques(),e.first,_langue);
	...
	if(!immu_baisse_statistique_si_statut.isEmpty()){
		...
		foreach(StatistiqueStatut e,immu_baisse_statistique_si_statut){
			...
			if(Utilitaire::traduisible(_d->val_noms_statuts(),e.second)){
				args_<<Utilitaire::traduire(_d->val_noms_statuts(),e.second,_langue);
			}else{
				args_<<Utilitaire::traduire(_d->val_noms_pseudo_statuts(),e.second,_langue);
			}
			retour_+=Utilitaire::formatter(_descriptions_capacites_.valeur(cle_).split("\t")[_langue],args_)+"\n";
		}
	}
	...
	if(degats_statut.first!=""){
		cle_="DEGATS_STATUT";
		QStringList args_;
		if(Utilitaire::traduisible(_d->val_noms_statuts(),degats_statut.first)){
			args_<<Utilitaire::traduire(_d->val_noms_statuts(),degats_statut.first,_langue);
		}else{
			args_<<Utilitaire::traduire(_d->val_noms_pseudo_statuts(),degats_statut.first,_langue);
		}
	...
	if(!cles_ch_.isEmpty()){
		...
		foreach(QString e,cles_ch_){
			if(Utilitaire::traduisible(_d->val_noms_statuts(),e)){
				evts_<<Utilitaire::traduire(_d->val_noms_statuts(),e,_langue)+","+Taux(loi_.proba_event(e),som_).chaine();
			}else{
				evts_<<Utilitaire::traduire(_d->val_noms_pseudo_statuts(),e,_langue)+","+Taux(loi_.proba_event(e),som_).chaine();
			}
		}
		QStringList args_;
