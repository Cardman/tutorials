...
Taux MonteCarloNombre::esperance_reelle()const{
	...
	if(quotient_reste_.second>Entier(0)){
		...
		for(int i=0;i<nb_evenements_;i++){
			Entier& effectif_=loi_2_.valeur(evenements_[i]);
			Entier tmp_=effectif_;
			effectif_*=quotient_reste_.first;
			if(i<max_indice_){
				effectif_+=tmp_;
				...
				effectif_+=quotient_reste_.second-somme_den_2_;
			}
		}
		loi_=loi_2_;
	}
...
MonteCarlo<bool> MonteCarloNombre::loi_sachant_sup(const Taux& _event)const{
	MonteCarlo<bool> loi_;
	if(!events().contains(_event)){
		loi_.ajouter_event(QPair<bool,Entier>(true,Entier(1)));
		return loi_;
	}
	Entier somme_1_(0);
	Entier somme_2_=proba_event(_event);
	foreach(Taux e,events()){
		if(e>_event){
			somme_1_+=proba_event(e);
			somme_2_+=proba_event(e);
		}
	}
	loi_.ajouter_event(QPair<bool,Entier>(true,somme_1_));
	loi_.ajouter_event(QPair<bool,Entier>(false,somme_2_-somme_1_));
	return loi_;
}
