...
const HashMap<QString,QStringList> DialogueLargeur::_elements_ihm_=DialogueLargeur::init_elements_ihm();

HashMap<QString,QStringList> DialogueLargeur::init_elements_ihm(){
...
QString DialogueLargeur::traduire(const QString& _chaine)const{
	return _elements_ihm_.valeur(_chaine)[langue];
}

DialogueLargeur::DialogueLargeur(int _langue,int _largeur,QWidget *_parent):QDialog(_parent){
	langue=_langue;
...
void DialogueLargeur::fin_ecriture_dims(){
	if(PanneauEditerEnv::_environnement_en_cours_!=NULL){
		emit dims(QPair<int,int>(largeur->value(),PanneauEditerEnv::_environnement_en_cours_->dimensions().second));
	}else{
		emit dims(QPair<int,int>(largeur->value(),PanneauDresseurLigue::_environnement_ligue_en_cours_->dimensions().second));
	}
}

int DialogueLargeur::largeur_v()const{
