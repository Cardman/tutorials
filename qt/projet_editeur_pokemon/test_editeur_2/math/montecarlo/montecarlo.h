...
void MonteCarlo<Element>::simplifier(){
	...
	bool suppr_=false;
	foreach(Element e,cles_){
		Entier *ent_=loi.valeur(e,suppr_);
		(*ent_)=(*ent_)/pgcd_;
	}
}
...
template<typename Element>
QString MonteCarlo<Element>::chaine_bool(int _langue)const{
	QList<Element> evs_=events();
	Entier som_=somme();
	QString retour_;
	foreach(Element e,evs_){
		if(e){
			retour_+="("+Utilitaire::traduire_bis(Import::_constantes_non_num_,"VRAI",_langue+1);
		}else{
			retour_+="("+Utilitaire::traduire_bis(Import::_constantes_non_num_,"FAUX",_langue+1);
		}
		retour_+=","+Taux(proba_event(e),som_).chaine()+");";
	}
	retour_.chop(1);
	return retour_;
}
