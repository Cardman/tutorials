...
Ball::Ball(const QStringList& _effets,int _prix_achat):Objet(){
	...
	if(!effets_.isEmpty()){
		...
		QString taux_base_=Import::_constantes_.filter(QRegExp("^TAUX_BASE\t"))[0].split("\t")[1];
	...
	if(!effets_.isEmpty()){
		...
		foreach(QString e,effets_){
			if(!e.contains("INFINI")){
				...
				Taux b_inf_=Taux::parse_taux(b_inf_ch_);
				ind_=e.indexOf(exp_taux_,ind_+b_inf_ch_.size()+1);
				Taux b_sup_=Import::_rapport_max_masse_;
				taux_masse.ajouter_cle_valeur(QPair<Taux,Taux>(b_inf_,b_sup_),Taux::parse_taux(exp_taux_.capturedTexts()[0]));
			}
		}
	}
	...
	if(!effets_.isEmpty()){
		...
		foreach(QString c,Import::_noms_envir_){
			QList<QString> cles_=taux_lieu.cles();
			QString lieu_=c.split("\t")[Import::_indice_francais_];
...
QString Ball::description(int _langue)const{
	QString retour_;
	QString cle_;
	if(taux_fixe!=Taux(0)){
		QStringList args_;
	...
	if(!cles_ch_.isEmpty()){
		...
		foreach(QString l,cles_ch_){
			QStringList args_;
			args_<<Utilitaire::traduire(Import::_noms_envir_,l,_langue);
	...
	if(!taux_pierre_evo.first.isEmpty()){
		...
		foreach(QString p,cles_ch_){
			ls_pierre_evo_<<Utilitaire::traduire(Import::_noms_pierres_evo_,p,_langue);
		}
		ls_pierre_evo_.sort();
	...
	if(!taux_types.first.isEmpty()){
		...
		QStringList ls_pierre_evo_;
		foreach(QString p,cles_ch_){
			ls_pierre_evo_<<Utilitaire::traduire(Import::_noms_types_,p,_langue);
		}
		ls_pierre_evo_.sort();
		foreach(QString p,ls_pierre_evo_){
			QStringList args_;
