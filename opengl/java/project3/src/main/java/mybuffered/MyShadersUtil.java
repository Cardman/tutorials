...
public final class MyShadersUtil {
	...
	public static ShaderIds loadShader(GL2 _gl, String _vertexShader, String _fragShader) {
	    ...
	    int infoLogLength;
	    _gl.glGetProgramiv(programId_, GL3.GL_INFO_LOG_LENGTH, infoLogLengthBuf);
	    infoLogLength = infoLogLengthBuf.get(0);
	    if (infoLogLength > 1) {
	    	ByteBuffer byteBuffer = ByteBuffer.allocate(infoLogLength);
	    	_gl.glGetProgramInfoLog(programId_, infoLogLength, infoLogLengthBuf, byteBuffer);
	      for (byte b:byteBuffer.array()){
	        System.err.print((char)b);
	      }
	      return null;
	    }
//        printProgramInfoLog(_gl, programId_);
...
//            System.out.println(logSize_[0]);
	...
	private static void printProgramInfoLog(GL2 _gl, int obj) {
	    GL2 gl = _gl; // get the OpenGL 3 graphics context
	    IntBuffer infoLogLengthBuf = IntBuffer.allocate(1);
	    int infoLogLength;
	    gl.glGetProgramiv(obj, GL3.GL_INFO_LOG_LENGTH, infoLogLengthBuf);
	    infoLogLength = infoLogLengthBuf.get(0);
