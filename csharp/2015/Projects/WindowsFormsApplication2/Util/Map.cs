...
namespace Util
{
    public class Map<K, V> : ListableEntries<K, V>, ListableKey//Dictionary<K,V>, IDictionary, 
    {
        [NonSerialized()]
        ...
        public IMyList<V> values()
        {
            MyList<V> list_ = new MyList<V>();
            foreach (Entry<K, V> e in list)
            {
        ...
        {
            List<IEntry<object, object>> list_ = new List<IEntry<object, object>>();
            foreach (Entry<K, V> e in list)
            {
                list_.Add(new Entry<object, object>(e.getKey(), e.getValue()));
            }
        ...
        public bool Contains(object _key)
        {
            if (_key == null)
            {
                foreach (Entry<K, V> e in list)
    ...
    public interface IEntry<out K, out V> : IEntry
    {
        K getKey();
        V getValue();
