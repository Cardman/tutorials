...
Taux::Taux(){
	numerateur=Entier(0);
	denominateur=EntierPositif(Entier(1));
}
...
void Taux::simplifier(){
	Entier pgcd_c_=numerateur.pgcd(Entier(denominateur));
	numerateur=numerateur/pgcd_c_;
	denominateur=EntierPositif(Entier(denominateur)/pgcd_c_);
}
...
Taux Taux::operator^(const Entier& _expo)const{
	if(!(_expo<Entier(0))){
		return Taux(numerateur^_expo,numerateur.non_negatif()||_expo%Entier(2)==Entier(0),Entier(denominateur)^_expo);
	}
	return Taux(Entier(denominateur)^(-_expo),numerateur.non_negatif()||_expo%Entier(2)==Entier(0),numerateur^(-_expo));
}
...
Taux& Taux::operator+=(const Taux& _autre){
	Entier ppcm_den_=Entier(denominateur).ppcm(Entier(_autre.denominateur));
	numerateur=numerateur*ppcm_den_/denominateur+_autre.numerateur*ppcm_den_/_autre.denominateur;
	denominateur=EntierPositif(ppcm_den_);
	//simplifier()
	return (*this);
}

Taux& Taux::operator*=(const Taux& _autre){
	numerateur=numerateur*_autre.numerateur;
	denominateur=EntierPositif(denominateur*_autre.denominateur);
	simplifier();
	return (*this);
}
...
bool operator<(const Taux& _celui_ci,const Taux& _autre){
	...
	return _celui_ci.gnumerateur()*Entier(_autre.gdenominateur())<Entier(_celui_ci.gdenominateur())*_autre.gnumerateur();
}
...
Entier Taux::partie_entiere()const{
	if(numerateur.non_negatif()){
		return numerateur/Entier(denominateur);
	}
	return -(numerateur/Entier(denominateur)+Entier(1));
}
...
Taux::operator unsigned long long(){
