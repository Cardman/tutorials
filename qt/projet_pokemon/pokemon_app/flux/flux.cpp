...
Flux& operator<<(Flux& _out,const qint8& _qint8){
	...
	/*if(_qint8>0){
		_out.lect_ecr+=QChar(_qint8);
	}else{
		_out.lect_ecr+=QChar(_qint8+UCHAR_MAX+1);
	}*/
	return _out;
}
...
Flux& operator>>(Flux& _in,qint8& _qint8){
	...
	_qint8=(qint8)elt_;*/
	_qint8=(qint8)_in.lect_ecr.mid(_in.position,2).toInt(NULL,16);
	_in.position+=2;
	/*if(_in.position>=_in.taille){
		throw "fin";
	}
	int elt_=_in.lect_ecr[_in.position].unicode();
	if(elt_>CHAR_MAX){
		elt_-=UCHAR_MAX+1;
	}
	_qint8=(qint8)elt_;
...
Flux& operator>>(Flux& _in,quint8& _quint8){
	...
	_quint8=(qint8)_in.lect_ecr.mid(_in.position,2).toInt(NULL,16);
	_in.position+=2;
	/*if(_in.position>=_in.taille){
		throw "fin";
	}
