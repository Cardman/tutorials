...
QString ObjetAttachableCombat::description(int _langue)const{
	QString retour_;
	QString cle_;
	if(annule_immu_type){
	...
	if(!sans_effet_capacite.isEmpty()){
		...
		foreach(QString t,sans_effet_capacite){
			args_<<Utilitaire::traduire(Import::_noms_capacites_,t,_langue);
		}
		args_.sort();
		args_=QStringList(args_.join(","));
		retour_+=Utilitaire::formatter(_descriptions_objets_.valeur(cle_).split("\t")[_langue],args_)+"\n";
	...
	if(statut_fin_tour_lanceur!=""){
		...
		QStringList args_;
		if(Utilitaire::traduisible(Import::_noms_statuts_,statut_fin_tour_lanceur)){
			args_<<Utilitaire::traduire(Import::_noms_statuts_,statut_fin_tour_lanceur,_langue);
		}else{
			args_<<Utilitaire::traduire(Import::_noms_pseudo_statuts_,statut_fin_tour_lanceur,_langue);
		}
		retour_+=Utilitaire::formatter(_descriptions_objets_.valeur(cle_).split("\t")[_langue],args_)+"\n";
	}
	...
	if(!cles_ch_.isEmpty()){
		...
		args_<<Utilitaire::traduire(Import::_noms_types_,type_,_langue);
	...
	if(!cles_cpl_.isEmpty()){
		...
		foreach(CleChCh e,cles_cpl_){
			...
			args_<<Utilitaire::traduire(Import::_noms_pk_,e.first,_langue);
	...
	if(!cles_cpl_.isEmpty()){
		...
		foreach(CleChCh e,cles_cpl_){
			...
			args_<<Utilitaire::traduire(Import::_noms_pk_,e.second,_langue);
