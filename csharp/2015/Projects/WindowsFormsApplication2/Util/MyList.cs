...
namespace Util
{
    ...
    public class MyList<T> : List<T>, IMyList<T>
    {
        ...
        public IMyList<T> sub(int _from, int _to)
        {
            if (_from > _to)
            {
                return new MyList<T>();
        ...
        public IMyList<T> subList(int _from, int _to)
        {
            if (_from > _to)
            {
                return new MyList<T>();
        ...
        public override bool Equals(object obj)
        {
            ...
            MyList<T> l_ = obj as MyList<T>;
            int minimumSize_ = Math.Min(size(), l_.size());
            for (int i = List.FIRST_INDEX; i < minimumSize_; i++)
            {
                if (get(i) == null && l_.get(i) != null)
                {
                    return false;
                } else if (!get(i).Equals(l_.get(i)))
                {
                    return false;
                }
            }
    ...
    public interface IMyList<out T> : IEnumerable<T>, IEnumerable, ICollection
    {
        T get(int _index);
        int size();
        ...
        bool isEmpty();
        int getLastIndex();
        IMyList<T> subList(int _from, int _to);
    }
    public class EnumList<T> : MyList<T> where T : struct, IConvertible, IFormattable, IComparable<T>
    {

    }
    public static class ExtLists
    {
        public static object getObj(this IEnumerable _list, int _index)
        {
            int i_ = 0;
