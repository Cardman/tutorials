...
QString Baie::description(int _langue)const{
	QString retour_;
	QString cle_;
	if(sans_echec){
		cle_="SANS_ECHEC";
	...
	if(!cles_ch_.isEmpty()){
		...
		foreach(QString l,cles_ch_){
			QStringList args_;
			args_<<Utilitaire::traduire(Import::_noms_types_,l,_langue);
	...
	if(!cles_ch_.isEmpty()){
		...
		foreach(QString l,cles_ch_){
			QStringList args_;
			args_<<Utilitaire::traduire(Import::_noms_categories_,l,_langue);
	...
	if(!soin_statut.isEmpty()){
		...
		foreach(QString l,cles_ch_){
			QStringList args_;
			if(Utilitaire::traduisible(Import::_noms_statuts_,l)){
				args_<<Utilitaire::traduire(Import::_noms_statuts_,l,_langue);
			}else{
				args_<<Utilitaire::traduire(Import::_noms_pseudo_statuts_,l,_langue);
			}
			retour_+=Utilitaire::formatter(_descriptions_baies_.valeur(cle_).split("\t")[_langue],args_)+"\n";
		}
	}
	...
	if(!cles_ch_.isEmpty()){//HashMap<QString,QPair<Taux,qint8> >
		...
		foreach(QString l,cles_ch_){
			...
			args_<<Utilitaire::traduire(Import::_noms_statistiques_,l,_langue);
